...
%        joint_reaction_constraint     between humerus head and glenoid
%
%
...
motion_file_name = 'flx1_1.mot';
print_flag = true;
withviz = false;

weight_coord = 1.5e-2;        % weight used in the tracking of the scapula
...
ikSetupFile = [path_to_repo, '\Code\Scale Model\Utilities\setup_files\Standford2019_experiments\IKSetup_2019.xml'];
...
% Waterloo's marker data is recorded at 50 Hz. 
timeInterval = 10;
...
options = optimoptions('fmincon','Display','none', ...
     ...
     'MaxIter',10000,'Algorithm','sqp', 'DiffMinChange', 1.0e-3);
...
epsilon = 0;
w = [ones(1,numMuscles), epsilon*ones(1,numCoordActs)];
w = [ones(1,numMuscles), epsilon*ones(1,8), 5*ones(1,6)];
cost =@(x) sum(w.*(x.^2));
...
for i = 1:numTimePoints
    ...
    [x,fval,exitflag,output] = fmincon(cost, x0, [], [], [], [], lb, ub, @(x)accelJntrxnCons(x, params, 1), options);
    if exitflag ==0
        [x,fval,exitflag,output] = fmincon(cost, x, [], [], [], [], lb, ub, @(x)accelJntrxnCons(x, params, 1), options);
        exitflag
    end
    ...
    % Retrieve optimal accelerations -> override muscle forces
    ...
    % Retrieve optimal accelerations: set the coordinate actuator controls
...
% activations that are found, together with other useful graphs.
...
name_file = append('muscle_activations_', subject_considered, '_experimentJRX_', trc_file_used(1));
muscle_order = "";
for i = 1:numMuscles
...
% rescale the frequency of the solution knowing that Waterloo's one is 50 Hz
