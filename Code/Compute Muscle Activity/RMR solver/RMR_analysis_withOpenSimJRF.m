function [optimizationStatus, unfeasibility_flags, tOptim, file_results] = RMR_analysis_withOpenSimJRF(subject_considered, model, trc_file, motion_file, weight_coord, time_interval, dynamic_activation_bounds, flag_GH_enforced, saving_path)
...
% * model: model to be used in the solver 
%          (valid thoracoscapular model with GH markers and coordinate actuators)
...
%                        considered, containing 0 if RMR is solved and 1 if 
%                        it is unfeasible
...
if strcmpi(experiment_name(1:5), 'shrug')
    model.getCoordinateSet().get('plane_elv').set_default_value(-0.433725);
    model.getCoordinateSet().get('plane_elv').set_locked(true);
    model.getCoordinateSet().get('axial_rot').set_default_value(0.8125346);
    model.getCoordinateSet().get('axial_rot').set_locked(true);
end
profile on
tic
...
for time_instant = 1:numTimePoints
    % state.setTime(rigth_time)   % have the same time as the trc or motion_files/sto_files
    if print_flag
    fprintf('.');
    ...
    model.realizeVelocity(state);   % model.assemble() is called here?
    ...
    params.glen = glen;
    params.Vec_H2GC = Vec_H2GC;

    q_ddot_0 = findInducedAccelerations(zeros(1,num_acts), params);
    delQ_delX = eye(num_acts);

    for k = 1:num_acts
        incrementalForceAccel_k= findInducedAccelerations(delQ_delX(k,:),params);
    ...
    if flag_GH_enforced
        [x,~,exitflag,output] = fmincon(cost, x0, [], [], A_eq_acc, Beq, lb, ub, @(x)jntrxncon_OpenSimJRF(x, params), options);
        if exitflag ==0
            % call the solver again, starting from current x, in case the maximum iterations are exceeded
            [x,~,exitflag,output] = fmincon(cost, x, [], [], A_eq_acc, Beq, lb, ub, @(x)jntrxncon_OpenSimJRF(x, params), options);
        end
        if exitflag<0 && time_instant>1
            ...
            [x,~,exitflag,output] = fmincon(cost, xsol(time_instant-1, :), [], [], A_eq_acc, Beq, lb, ub,  @(x)jntrxncon_OpenSimJRF(x, params), options);
        end
    else
    ...
    if print_flag
        ...
        simulatedAccelerations(time_instant,:) = findInducedAccelerations(x,params);
